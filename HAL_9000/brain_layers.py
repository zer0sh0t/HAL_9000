...
'''
class LSTM(Layer):
    def __init__(self, n_units, input_shape=None, activ1='sigmoid', activ2='tanh', trunc=5):
        self.input_shape = input_shape
        self.n_units = n_units
        self.sig_activ = activ_fns[activ1]()
        self.t_activ = activ_fns[activ2]()
        self.trainable = True
        self.trunc = trunc
    ...
    def init_parameters(self, opt):
        timesteps, input_dim = self.input_shape
        i = 1 / math.sqrt(input_dim)
        self.Wxh_i = np.random.uniform(-i, i, (self.n_units, input_dim))
        self.Wxh_f = np.random.uniform(-i, i, (self.n_units, input_dim))
        self.Wxh_o = np.random.uniform(-i, i, (self.n_units, input_dim))
        self.Wxh_g = np.random.uniform(-i, i, (self.n_units, input_dim))

        i = 1 / math.sqrt(self.n_units)
        self.Whh_i = np.random.uniform(-i, i, (self.n_units, self.n_units))
        self.Whh_f = np.random.uniform(-i, i, (self.n_units, self.n_units))
        self.Whh_o = np.random.uniform(-i, i, (self.n_units, self.n_units))
        self.Whh_g = np.random.uniform(-i, i, (self.n_units, self.n_units))

        self.Whh_opt = copy.copy(opt)
        self.Wxh_opt = copy.copy(opt)

    def n_parameters(self):
        return np.prod(self.Whh_i.shape) + np.prod(self.Wxh_i.shape) + np.prod(self.Whh_f.shape) + np.prod(self.Wxh_f.shape) + np.prod(self.Whh_o.shape) + np.prod(self.Wxh_o.shape) + np.prod(self.Whh_g.shape) + np.prod(self.Wxh_g.shape) +

    def forward_propagation(self, X, training=True):
        self.layer_input = X
        batch_size, timesteps, input_dim = X.shape

        self.i_ba = np.zeros((batch_size, timesteps+1, self.n_units))
        self.i = np.zeros((batch_size, timesteps, self.n_units))

        self.f_ba = np.zeros((batch_size, timesteps+1, self.n_units))
        self.f = np.zeros((batch_size, timesteps, self.n_units))

        self.o_ba = np.zeros((batch_size, timesteps+1, self.n_units))
        self.o = np.zeros((batch_size, timesteps, self.n_units))

        self.g_ba = np.zeros((batch_size, timesteps+1, self.n_units))
        self.g = np.zeros((batch_size, timesteps, self.n_units))

        self.c_ba = np.zeros((batch_size, timesteps+1, self.n_units))
        self.c = np.zeros((batch_size, timesteps, self.n_units))

        self.h = np.zeros((batch_size, timesteps, self.n_units))

        self.h[:, -1] = np.zeros((batch_size, self.n_units))
        self.c[:, -1] = np.zeros((batch_size, self.n_units))

        for t in range(timesteps):
            self.i_ba[:, t] = self.h[:, t -
                                     1].dot(self.Whh_i.T) + X[:, t].dot(self.Wxh_i.T)
            self.i[:, t] = self.sig_activ(self.self.i_ba[:, t])

            self.f_ba[:, t] = self.h[:, t -
                                     1].dot(self.Whh_f.T) + X[:, t].dot(self.Wxh_f.T)
            self.f[:, t] = self.sig_activ(self.self.f_ba[:, t])

            self.o_ba[:, t] = self.h[:, t -
                                     1].dot(self.Whh_o.T) + X[:, t].dot(self.Wxh_o.T)
            self.o[:, t] = self.sig_activ(self.self.o_ba[:, t])

            self.g_ba[:, t] = self.h[:, t -
                                     1].dot(self.Whh_g.T) + X[:, t].dot(self.Wxh_g.T)
            self.g[:, t] = self.sig_activ(self.self.g_ba[:, t])

            self.c_ba[:, t] = (self.f[:, t] *
                               self.c[:, t-1]) + (self.i[:, t] * self.g[:, t])
            self.c[:, t] = self.t_activ(self.c_ba[:, t])

            self.h[:, t] = self.o[:, t] * self.c[:, t]
    ...
    def backward_propagation(self, grad):
        ...
        grad_Wxh_i = np.zeros_like(self.Wxh_i)
        grad_Whh_i = np.zeros_like(self.Whh_i)

        grad_Wxh_f = np.zeros_like(self.Wxh_f)
        grad_Whh_f = np.zeros_like(self.Whh_f)

        grad_Wxh_o = np.zeros_like(self.Wxh_o)
        grad_Whh_o = np.zeros_like(self.Whh_o)

        grad_Wxh_g = np.zeros_like(self.Wxh_g)
        grad_Whh_g = np.zeros_like(self.Whh_g)

        grad_ = np.zeros_like(grad)
        for t in range(timesteps):
            grad_c = grad[:, t] * self.o[:, t] * \
                self.t_activ.gradient(self.c_ba[:, t])
            grad_f = grad_c * self.sig_activ(self.f_ba[:, t])
